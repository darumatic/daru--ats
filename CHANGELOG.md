...
#### Added
- Candidate match scores are now weighted, editable criteria rather than a fixed formula. `Admin Area > Match Criteria` seeds JD Criteria Match, Location, Local Experience, Big Company and University, each with a weight and an evaluator, and any of them can be renamed, reweighted, added to or removed. A job order can specialise the set for its own role (`Use the default template` off), which snapshots the criteria so later template edits cannot rewrite a role someone has tuned; the job-order view offers to re-apply the template when it has moved on.
- A criterion the rules engine cannot honestly judge is reported as `not assessed` instead of scored zero, and is excluded from the weighted average. Every match now carries a coverage figure alongside the score, so `72% from 3 of 5 criteria` is legible as such, and a candidate nothing could be assessed for reads `Not scored` rather than 0%. Out of the box, Big Company and University are unassessed until you list the employers or schools that matter to you (or run an AI pass), so a stock install reports around 60-75% coverage by design.
- `Score with AI` on a match row, and `Score All With AI` on a job order (at most 25 candidates per run, 3 at a time), ask the model to judge the criteria the rules engine could not - persisted per candidate/job pair and shown in place of the rule score. Cached judgements are stamped per criterion, so re-weighting a criterion keeps them and only redefining one invalidates it.
- `Admin Area > System Settings` gained an optional `Reasoning Model`, used only by candidate scoring. Blank means every AI feature keeps using the standard model, so no install starts paying for a reasoning model by accident.
- The AI features can now run on Google Gemini instead of OpenAI. `Admin Area > System Settings > Platform Settings` gained `AI Provider` and `AI Model` fields next to the (renamed) `AI API Key`. Gemini is reached through Google's OpenAI-compatible endpoint, so both providers share one request path. Existing installs keep working unchanged: a row with no provider stored reads as OpenAI on its previous model.
...
#### Fixed
- Each match criterion's guidance (`What should this judge?`) can now be edited. It is what the AI is sent as the instruction for that criterion, but the editor had no field for it, the admin save dropped it, and specialising a job order lost it - so only the five seeded criteria had any guidance and none of it could be changed.
- `POST /api/match-explanations` no longer takes the match score from the request body. The score, reasons and risks are recomputed server-side before the explanation is generated, so the prose and the number cannot disagree and neither is caller-controlled; previously any caller could write any score and have the model explain it.
- AI requests now have a timeout (60s, or 180s for a reasoning model). There was none at all, so a provider that accepted the connection and then went quiet held the request until the platform killed it. A timeout is logged as its own `ai.request.timeout` event rather than as an unavailable provider.
- Reasoning models are no longer sent a temperature they reject. The client leaves the field out for the model families known to refuse it, and - because that list goes stale as new models ship - also retries once without it when a provider rejects it by name, logging `ai.request.temperature_rejected`.
- Candidate matching no longer scans the entire in-scope candidate pool on every request. The scan is capped (`MATCH_LIST_MAX_CANDIDATE_POOL`, default 500, most recently updated first). **This is a behaviour change as well as a fix:** a division with more candidates than the cap will no longer see the bottom of the list.
- AI provider failures are no longer silent. The shared AI client logs every failed call with the provider's own reason - `ai.request.failed` with provider, model and HTTP status, plus `ai.response.empty`, `ai.response.invalid_json` and `ai.request.unavailable`, and `ai.request.schema_rejected` as a warning when the plain-JSON retry recovers - with the API key stripped from the logged detail. Resume parsing degrades to the built-in parser on failure and answers 200 with a warning, so a dead key or an unknown model previously looked exactly like an ordinary parse.
- `OPENAI_RESUME_MODEL` is no longer applied to a provider it cannot name. The variable overrides the default model only while the provider is OpenAI; on Gemini, a blank `AI Model` now resolves to the Gemini default as the field's hint promises. Previously an install that still carried the variable sent `gpt-4o-mini` to Google and every AI feature failed.
- The resume parse response names the parser that actually ran (`openai`, `gemini` or `fallback`) instead of reporting `openai` for every AI parse.
- A system-settings read failure can no longer pass unnoticed. `readSystemSettingRecord()` now separates "no settings row yet" from "the read failed": the failure is logged instead of swallowed, `/api/health` reports `systemSettings` and returns 503 so the deploy's rollback gate can see it, `PATCH /api/system-settings` returns 503 rather than writing the blank form over stored credentials and branding, and `uploadObjectBuffer()` refuses the local-disk fallback when the storage config could not be read (the fallback still applies when object storage is genuinely unconfigured). Previously a pending migration made every full read of the row throw into a bare `catch`, silently reverting branding to defaults and switching the public careers site off while health still reported 200.
- `robots.txt` and `sitemap.xml` are now rendered at request time. Both read system settings, and system-settings reads are deliberately skipped during `next build`, so as statically generated routes they baked in the "career site disabled" variant - publishing `Disallow: /careers` and an empty sitemap even while the careers site was live.

#### Changed
- **Existing match percentages will move on this release.** The score is now the weighted criteria set rather than the previous fixed blend, and criteria that cannot be assessed are excluded rather than guessed. The old formula survives as the JD Criteria Match evaluator (the heaviest criterion by default) so the ordering of a typical job stays broadly recognisable, but individual numbers will differ.
- Location scoring now uses the coordinates both candidate and job order already store, scored against the criterion's radius, and falls back to the previous city/state text comparison only when either side lacks them.
- The two match routes no longer carry separate copies of the scoring heuristics. They shared 171 byte-identical lines, which could drift apart silently; both now import `lib/match-scoring.js`, and a test pins that both directions report the same score for the same candidate/job pair.
- The seven AI features now share one HTTP client (`lib/ai-chat-client.js`) instead of each holding its own copy of the endpoint, key check, retry and response handling. Two internal payloads were renamed with it: `/api/health` reports `ai`, `aiProvider` and `aiModel` (was `openAi` and `openAiResumeModel`), and the admin data export reports `hasAiApiKey` (was `hasOpenAiApiKey`).
- Closed all open Dependabot alerts and cleared `npm audit`. `nodemailer` 9.0.1 -> 9.1.1 (recipient-domain validation bypasses via IDN/punycode and RFC 5322 comments, `resolveContent()` file/URL access bypass, addressparser ReDoS), `sanitize-html` 2.17.5 -> 2.17.7 (stored XSS via SVG SMIL and a `</textarea/>` mutation-XSS bypass), `mysql2` 3.22.5 -> 3.24.4 (decompression-bomb DoS), plus transitive `@xmldom/xmldom` 0.8.15, `browserslist` 4.28.9 and `baseline-browser-mapping` 2.11.21, and `vitest` 4.1.11 for the dev toolchain.
- The candidate forms no longer require `Division`, `Owner`, `Current Job Title` or `Current Employer`. Only first name, last name, email, mobile, stage and source are mandatory, so a lead can be captured before those details are known; division and owner still default to the acting user's division on save, and the server keeps enforcing an owner where the division access mode is `Owner Only`.
