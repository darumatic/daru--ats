...
import { normalizeAiProvider, getAiProviderProfile } from '@/lib/ai-providers';
...
function resolveAiModel(setting) {
	return (
		asTrimmedString(setting?.aiModel)
		|| asTrimmedString(process.env.OPENAI_RESUME_MODEL)
		|| getAiProviderProfile(setting?.aiProvider).defaultModel
	);
}
