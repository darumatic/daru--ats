...
import { jobOrderMatchCriteriaSchema } from '@/lib/match-criteria';
...
export const jobOrderSchema = z
	.object({
		...
		applicationQuestions: z
			...
			.optional()
			.default([]),
		// Deliberately .optional() with no default, unlike applicationQuestions:
		// absent has to mean "leave the stored value alone" while an explicit
		// null means "stop specialising and go back to the template". A default
		// would collapse those two into one.
		matchCriteria: jobOrderMatchCriteriaSchema.optional()
	})
